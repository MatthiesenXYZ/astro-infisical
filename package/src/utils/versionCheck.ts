    ...
    async (
        ...
        if (params.command === 'dev') {
            ...
            try {
                ...
                if (comparison === -1) {
                    logger.warn(`A new version of ${opts.name} is available. Please update to ${latestVersion} using your favorite package manager.`);
                ...
                if (error instanceof Error) {
                    ...
                    logger.error('An unknown error occurred while fetching the latest version from the npm registry.');
                }
            }
        }
});