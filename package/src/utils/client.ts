...
export const infisicalClient = ( clientOpts: { siteUrl: string, clientId: string, clientSecret: string } ) => {
    return {
        makeClient: () => new InfisicalClient({
        ...
        getSecrets: async (secretOpts: { environment: string, projectId: string, path: string, processEnv: boolean } ) => {
